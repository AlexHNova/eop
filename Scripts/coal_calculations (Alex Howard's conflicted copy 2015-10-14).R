...
calculate <- function(coal_df) {
  for (k in 2:nrow(coal_df)) {
  m <- (k - 1) 
  
  coal_df[k, "cumulative_coal_kg"] <- ifelse(is.na(coal_df[m, "cumulative_coal_kg"]), 0, coal_df[m, "cumulative_coal_kg"]) + 
                                ifelse(is.na(coal_df[k, "kg_bought"]), 0, coal_df[k, "kg_bought"])
  coal_df[k, "cumulative_usage_kg"] <-  ifelse(is.na(coal_df[k, "cumulative_coal_kg"]), 0, coal_df[k, "cumulative_coal_kg"]) - 
                                  ifelse(is.na(coal_df[k, "current_coal_weight"]), 0, coal_df[k, "current_coal_weight"])
  coal_df[k, "usage_net_kg"] <- ifelse(is.na(coal_df[k, "cumulative_usage_kg"]), 0, coal_df[k, "cumulative_usage_kg"]) - 
                          ifelse(is.na(coal_df[m, "cumulative_usage_kg"]), 0, coal_df[m, "cumulative_usage_kg"])
  }
...
check_coal <- function(df) {
  ...
  while (!OKAY) {
    ...
    for (r in 1:nrow(df)) {
      # if the net usage is positive for that record, it is okay, so move on
      if (df[r, "usage_net_kg"] >= 0) { next }
      
      # reaching this point means the net usage is negative, so inspect
      # if it is negative by a small amount, i.e. between 0 and 7, the format weight is probably just underestimated
      if (df[r, "usage_net_kg"] > -7) {
        buying_format <- df[r, "coal_buying_format"]
        adjust_by <- ceiling((-1) * (df[r, "usage_net_kg"]) + 0.5)
        format_weights[[buying_format]] <- format_weights[[buying_format]] + adjust_by
        break
      }
      
      # if it is negative by a larger amount, it means that the number of units bought is probably incorrect
...
## correct the cbfs of stands 172 and 3152
stand_df_list[["172"]]$coal_buying_format <- "Small Bag"
stand_df_list[["3152"]]$coal_buying_format <- "Small Bag"

# --------------------- #
# w.r.t. the "units bought since last..." 
## - typos in data for stand 3466
s3466 <- stand_df_list[["3466"]]
...
stand_df_list[["172"]] <- s172
rm(s172)

# s1495 <- stand_df_list[["1495"]]
...
stand_df_list <- lapply(X = stand_df_list, FUN = function(s) {
  ...
  if (VERBOSE) {print(unique(as.character(s$stand_number)))}
